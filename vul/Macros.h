...
// licensing.  Third party software is not bundled with this software, but may be
// available from the licensor.  License hyperlinks are provided here for
...
#define VUL_ASSERT(boolean_statement, message)                                 \
  {                                                                            \
    if (not(boolean_statement)) {                                              \
      throw std::logic_error(                                                  \
          std::string("ASSERT_FAILED: ") + std::string(message) +              \
...
using HostPolicy = Kokkos::RangePolicy<HostExec>;
...
template <typename View> auto create_host_copy(View gpu) {
